...
def databaseWrite(databaseInput):
    ...
    query = "INSERT INTO networkTest11 (Key, Timestamp, SourceMAC, DestinationMAC, \
    TotalBits, FlagPassive, Flag2GHz, FlagOFDM, FlagCCK, FlagGFSK, Flag5GHz, FlagGSM, \
    FlagCCKOFDM, TotalPackets, SignalStrength, DataRate, Duration, PhysType) VALUES \
...
def databaseWriteList(databaseInput, table_time = "None", table_type = "None"):
    ...
    query = sql.SQL("INSERT INTO {} (Key, Timestamp, SourceMAC, DestinationMAC,\
    TotalBits, FlagPassive, Flag2GHz, FlagOFDM, FlagCCK, FlagGFSK, Flag5GHz,\
    FlagGSM, FlagCCKOFDM, TotalPackets, SignalStrength, DataRate, Duration, \
    DurationPreamble, PhysType) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, \
    %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)").format(sql.Identifier(table_time + table_type))   
...
def databaseWriteTable(table_time = None, table_type=None):
    ...
    if table_type is None:
        print("To create a table a table type must be defined. Use argument\
        table_type to specify")
        conn.close()
        return
    ...
    if tableExists(cur, table_name) is False:
        if table_type is not None:
            ...
            args = "(Key VARCHAR(30) PRIMARY KEY, Timestamp INT, SourceMAC VARCHAR(20),\
            DestinationMAC VARCHAR(20), TotalBits INT, FlagPassive INT, Flag2GHz INT, \
            FlagOFDM INT, FlagCCK INT, FlagGFSK INT, Flag5GHz INT, FlagGSM INT, \
            FlagCCKOFDM INT, TotalPackets INT, SignalStrength INT, DataRate INT, \
            Duration INT, DurationPreamble INT, PhysType VARCHAR(3))"
        else:
        ...
        cur.execute(query)
        print("Table created")
        #cur.execute("CREATE TABLE networkTest11 (Key VARCHAR(50) PRIMARY KEY,\
        # TimeStamp VARCHAR(20), SourceMAC VARCHAR(20), DestinationMAC VARCHAR(20),\
        # Bandwidth INT, FLAG1 INT, FLAG2 INT, FLAG3 INT, FLAG4 INT, FLAG5 INT, \
        # FLAG6 INT, FLAG7 INT, FLAG8 INT, FLAG9 INT, FLAG10 INT)")
    else:
